1. 磁盘基础知识
	...
	3. GPT分区格式（GUID Partition Table）
		- 最多可以有128个分区 , 主分区 , 扩展分区和逻辑分区的概念 , 每个分区都有一个GUID作为唯一标识符

2. 文件系统
	1. 简介：很多人忽略了，硬盘分好区后是不能直接使用的。有过装机经验的朋友知道，硬盘格式化时，会有个文件系统让你选择（NTFS,exFATd等windows默认是NTFS），这个文件系统就是操作系统中用于管理持久化数据的核心子系统，主要功能是将用户的文件组织并存储在磁盘中，即使断电也能保证数据的完整性。
	2. 文件系统类型
		1. 磁盘文件系统：ext2/3/4，xfs等，直接将数据存储在硬盘
		2. 内存文件系统：`/proc`, `/sys` 等，数据存储在内存中，主要用于访问内核信息
		3. 网络文件系统：如NFS和SMB，用于访问远程的主机数据
	3. Linux特定分区下创建文件系统，使用[[#11. mkfs 为分区创建文件系统|mkfs]]

4. **mount** 用于挂载文件系统，基本格式为 `mount -t type [-o options] device dir`。
	1. 文件系统了类型：
	2. 
	- mount  /dev/hda1 /mnt
...
##### 11. mkfs 为分区创建文件系统
1. 格式：`mkfs [-V] [-t 文件系统] [存储设备]`
	1. -V 显示详细信息
	2. -t 指定文件系统，默认为ext2
	3. -c 创建文件系统前检查分区是否有坏道
2. 例：
	1. `mkfs -t ext3 /dev/sda1` 格式化sda1为ext3
	2. mkfs.ext3 /dev/sda6       # 把该设备格式化成ext3文件系统
	3. mkfs.vfat /dev/sda6       # 格式化成fat32文件系统
