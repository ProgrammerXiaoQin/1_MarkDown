...
7. grep                                 关键词搜索
	- -n    显示匹配行跟行号
	- -i      忽略大小写
	- -A  number   查看关键字位置和关键词下面number行
	- -B  number   查看关键字位置和关键词上面number行
	- -C  number   查看关键字位置和关键词附件number行
...
### 十四.其他高级命令
1. **xargs** 从标准输入接受输入，并把输入转换为一个特 定命令的参数列表
	1. 参数说明
		- -i 将结果 一项项复制给{}
	2. 实例
```shell
