...
>使用大括号 {} 创建字典时，字符串键 key 用引号；而使用 dict() 创建字典时，字 符串键不使用引号
##### 4. Set 集合
...
5. 字符串前面加
	- `r或R` ：表示原始字符串（raw string）。在这种字符串中，反斜杠 `\` 不会被视为转义字符
	- `f` 或 `F`：表示格式化字符串（f-string）。这种字符串允许在大括号 `{}` 中直接嵌入变量或表达式。
	- `b` 或 `B`：表示字节字符串（bytes）。字节字符串用于处理二进制数据
...
# 5.视图，浅复制，深复制
1. 当用 = 直接赋值，是非拷贝方法，结果是产生一个视图 (view)。这两个列表是等价的，修改 其中任何 (原始列表、视图) 一个列表都会影响到另一个列表（用等号 = 赋值得到的List和原List共享同一地址）
2. 浅复制 (shallow copy) 只对 list 的第一层元素完成拷贝，深层元素还是和原 list 共用。
3. 深复制 (deep copy) 是创建一个完全独立的列表对象，该对象中的元素与原始列表中的元素是 不同的对象